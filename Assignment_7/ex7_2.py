...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def add_filter_table2(self, datapath):
        ...
        mod = parser.OFPFlowMod(datapath=datapath, table_id=TCP_FILTER_TABLE, 
                                priority=1, instructions=inst)
        datapath.send_msg(mod)
    ...
    def apply_filter_port(self, datapath):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        match = parser.OFPMatch(eth_type=ether_types.ETH_TYPE_IP, ip_proto=6,ipv4_src='10.0.0.3', ipv4_dst='10.0.0.4', tcp_dst=44000)
        mod = parser.OFPFlowMod(datapath=datapath, table_id=TCP_FILTER_TABLE,
                                priority=10000, match=match)
        datapath.send_msg(mod)
